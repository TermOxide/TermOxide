...
/// 2. Functions taking `Int` are self-documenting — it's clearly a CSS integer,
///    not an arbitrary `i32`.
/// 3. The proc_macro can distinguish `Int` from [`super::unit::Unit`] and
///    [`Float`] at the type level, preventing category errors.
///
/// # Examples
///
/// ```rust
/// use termoxide_layout::style::number::Int;
/// let z = Int::new(10);
/// let order = Int::ZERO;
/// ```
...
/// real-number property that isn't a spatial dimension (use
/// [`super::unit::Unit`] for those).
///
/// # Why not bare `f32`?
///
...
/// - `NaN == NaN` under bit equality (same payload). This is intentional: NaN
///   is not a valid style value and should never appear in production. If the
///   proc_macro validates inputs at parse time, NaN never reaches runtime.
///
/// # Examples
///
/// ```rust
/// use termoxide_layout::style::number::Float;
...
///
/// ```rust
/// use termoxide_layout::style::number::Opacity;
///
...
impl Opacity {
    ...
    pub const fn get(self) -> f32 { self.0.0 }

    /// Borrow the underlying [`Float`] (guaranteed clamped).
    pub const fn as_float(self) -> Float { self.0 }
}
...
impl From<Float> for Opacity {
    fn from(v: Float) -> Self { Self::new(v.0) }
}
