...
//!
//! - **`Option<T>` for unset style fields**: Distinguishing "not set" from
...
//!   to the type default. Almost every field in [`Style`] is `Option<T>`;
//!   the sole exception is `dimensions`, which is itself a struct of six
//!   `Option<Unit>` sub-fields and therefore doesn't need a second wrapper.
//!
pub mod box_model;
pub mod color;
pub mod font;
...
///
/// Most fields are `Option<T>`. `None` means **"not declared on this element"**.
/// This distinction drives three core behaviours:
///
...
///
/// ```rust
/// use termoxide_layout::{Style, box_model::Dimensions, color::{Color, NamedColor}, font::FontStyle, unit::Unit};
///
