...
//!
//! 1. **Modifiers travel end to end** — Ctrl-C and Ctrl-D quit, while a bare `c` or `d` only bumps the counter.
//! 2. **Resize still relayouts** — the stream carries no resize event, so the loop samples the viewport every frame
//!    instead. Resize the terminal while idle (without typing) and the reported size must follow.
//! 3. **The terminal is left clean** — on exit, no alternate screen and no residual raw mode. Type `blah` at the shell
