...
//! - No `key`, `classes`, `event_handler` on `ViewNode` yet — those are builder-only
//!   metadata that will be added to `ViewNode` when the reconciler and event-router land.
//!   For now the builder carries them and drops what `ViewNode` can't hold yet.
...
impl ContainerBuilder {
    ...
    pub fn children(
        mut self,
        children: impl IntoIterator<Item = impl IntoViewNode>,
    ) -> Self {
        self.children = children
            .into_iter()
            .map(IntoViewNode::into_view_node)
            .collect();
        self
    }
}
...
impl NodeBuilder for TextBuilder {
    ...
    fn build(self) -> ViewNode {
        let mut node =
            ViewNode::text(self.area, self.content, self.common.style);
...
pub fn el(_: Container) -> ContainerBuilder {
    ContainerBuilder::new()
}
pub fn text(content: impl Into<String>) -> TextBuilder {
    TextBuilder::new(content)
}
pub fn button(label: impl Into<String>, msg: Msg) -> ButtonBuilder {
    ButtonBuilder::new(label, msg)
}
